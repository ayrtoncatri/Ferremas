...
async function createProductWithImage(product, imageBuffer, callback) {
  try {
    ...
    getProductByCode(product.productCode, (err, existingProduct) => {
      if (err) {
        return callback(err);
      }
      ...
      if (existingProduct) {
        ...
        updateProduct(product.productCode, existingProduct, callback);
      } else {
...
function updateProduct(productCode, product, callback) {
  const query = `UPDATE products SET brand = ?, code = ?, name = ?, price = ?, image = ?, stock = ? WHERE productCode = ?`;
  const params = [
    product.brand,
    product.code,
    product.name,
